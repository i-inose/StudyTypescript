...
// ============================================================
// ここに回答 ↓
function q1Answer(a: number, b: number): number {
  return a + b; // この関数を完成させてください
}
...
// ============================================================
// ここに回答 ↓
const q2Answer = (s: string): number => {
  return s.length; // この関数を完成させてください
};
...
// ============================================================
// ここに回答 ↓
function q3Answer(name: string, greeting?: string): string {
  if (greeting) {
...
function q4Answer(price: number, taxRate: number = 0.1): number {
  return price * (1 + taxRate); // この関数を完成させてください（税込み価格を計算する）
}
...
// ============================================================
// ここに回答 ↓
function q5Answer(prefix: string, ...names: string[]): string[] {
  return [prefix, ...names]; // この関数を完成させてください
}
...
function q6Answer(
  numbers: number[],
  predicate: (n: number) => boolean, // ← 正しい関数型を書いてください
): number[] {
  return numbers.filter(predicate); // この関数を完成させてください
}
...
function runChapter04Tests(): ChapterResult {
  ...
  results.push(
    runTest(5, "レストパラメータ", () => {
      assert.deepEqual(q5Answer("Mr.", "田中", "鈴木"), [
        "Mr. 田中",
        "Mr. 鈴木",
      ]);
  ...
  results.push(
    runTest(6, "コールバック関数を引数に取る関数", () => {
      assert.deepEqual(
        q6Answer([1, 2, 3, 4, 5], (n: number) => n > 3),
        [4, 5],
      );
      assert.deepEqual(
        q6Answer([10, 20, 30], (n: number) => n < 25),
        [10, 20],
      );
      assert.deepEqual(
        q6Answer([], (n: number) => true),
        [],
      );
    }),
  );
  ...
  results.push(
    runTest(8, "オブジェクトを返すアロー関数", () => {
      assert.deepEqual(q8Answer("太郎", 20), {
        name: "太郎",
        age: 20,
        isAdult: true,
      });
      assert.deepEqual(q8Answer("花子", 15), {
        name: "花子",
        age: 15,
        isAdult: false,
      });
      assert.deepEqual(q8Answer("次郎", 18), {
        name: "次郎",
        age: 18,
        isAdult: true,
      });
