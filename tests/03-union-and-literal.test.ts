...
// ============================================================
// ここに回答 ↓
type Status = any; // ここを修正

const q2Answer: any = "pending";
...
// ============================================================
// ここに回答 ↓
function q5Answer(shape: any): number {
  return 0; // この関数を完成させてください
}
...
// ============================================================
// ここに回答 ↓
function q8Answer(state: any): string {
  return ""; // この関数を完成させてください
}
...
// ============================================================
// ここに回答 ↓
type CounterAction = any; // ここを修正

function q10Answer(state: number, action: any): number {
  return state; // この関数を完成させてください
}
...
function runChapter03Tests(): ChapterResult {
  ...
  results.push(runTest(1, "ユニオン型（string | number）", () => {
    assert.equal(q1a, 42);
    assert.equal(q1b, "hello");
  }));

  results.push(runTest(2, "リテラル型の定義", () => {
    assert.equal(q2Answer, "pending");
  }));

  results.push(runTest(3, "ユニオン型を使った関数", () => {
  ...
  results.push(runTest(4, "判別共用体の定義", () => {
  ...
  results.push(runTest(5, "判別共用体を switch で処理", () => {
  ...
  results.push(runTest(8, "API 状態に応じたメッセージ", () => {
    assert.equal(q8Answer({ status: "idle" }), "待機中");
    assert.equal(q8Answer({ status: "loading" }), "読込中");
    assert.equal(q8Answer({ status: "done", data: "完了データ" }), "完了データ");
  }));

  results.push(runTest(9, "exhaustive check", () => {
    assert.equal(q9Answer("red"), "#FF0000");
    assert.equal(q9Answer("green"), "#00FF00");
    assert.equal(q9Answer("blue"), "#0000FF");
  }));

  results.push(runTest(10, "reducer パターン（useReducer 風）", () => {
  ...
  return { chapter: "03章: ユニオン型・リテラル型・判別共用体", results, passed, total: results.length };
}
